...
import com.ixxus.hastalavista_refactor.search.PageIndexComponent
...
trait AnalyticsServiceComponent {
    ...
    class AnalyticServiceUsingHashMap private(private var data: Map[String, Analytics]) extends AnalyticsService {
           ...
           val pageOption = pageIndex(url)
            pageOption match {
                case Some(page) =>  Future {
                    AnalyticServiceUsingHashMap.clickCountQueue.put(page.url)
                }
                case None => throw new IllegalArgumentException(s"$url has not been indexed")
            }
        }

        private def updateClickCountImpl(url:String): Unit = {
            val existing = data.get(url)
            existing match {
                case Some(existingItem) =>
    ...
    object AnalyticServiceUsingHashMap {
        val clickCountQueue = new LinkedBlockingQueue[String]()

        def apply(data: Map[String, Analytics]): AnalyticServiceUsingHashMap = new AnalyticServiceUsingHashMap(data)
    }
}
